...
***Citations:https://www.rdocumentation.org/packages/gmodels/versions/2.19.1/topics/ci + Geom ribbon and shading CI's: https://www.geeksforgeeks.org/shading-confidence-intervals-manually-with-ggplot2-in-r/# + ifelse review: https://www.datamentor.io/r-programming/ifelse-function + loess review: https://r-statistics.co/Loess-Regression-With-R.html***
...
# filter for right conditions/women and reference time
dat_processed <- dat %>%
...
# CI functions, if 95 then L95 and U95 used, if 80 then L80 and U80 used (If use geom_smooth do something like p <- p + geom_smooth(method = "loess", se = TRUE something like that but bounds aren't defined :p - dan))
...
plot1 <- plot_cp(dat, est, iso_code = 4)
print(plot1)

plot2 <- plot_cp(dat, est, iso_code = 4, CI = NA)
print(plot2)

plot3 <- plot_cp(dat, est, iso_code = 404, CI = 80)
print(plot3)
```
...
https://github.com/CoffeeLover-872/advRHW1.git 
